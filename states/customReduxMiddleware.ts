...
import { ActionType as ActionTypeDetailThread, neutralVoteCommentDetailThreadActionCreator, neutralVoteDetailThreadActionCreator } from "./detailThread/action";
...
const customReduxMiddleware: Middleware = (store) => (next) => (action) => {
  ...
  if (action.type === ActionTypeDetailThread.UP_VOTE_DETAIL_THREAD || action.type === ActionTypeDetailThread.DOWN_VOTE_DETAIL_THREAD) {
    store.dispatch(neutralVoteDetailThreadActionCreator(store.getState().authUser.id));
  }
